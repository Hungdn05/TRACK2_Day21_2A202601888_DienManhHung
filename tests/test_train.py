...
def test_find_best_threshold_uses_configured_grid():
    y_true = np.array([0, 0, 1, 1])
    probabilities = np.array([0.10, 0.40, 0.45, 0.90])

    threshold, optimized_f1 = find_best_threshold(y_true, probabilities)

    assert any(np.isclose(threshold, value) for value in DECISION_THRESHOLDS)
    assert optimized_f1 == 1.0
